...
import Visit_Visa_Appointment from "./Visit_Visa_Appointment";
import VisaCheckList from "../AllVisa_Details_Tab/VisaCheckList";
import Visit_Visa_Checklist from "./Visit_Visa_Checklist";

const Visit_Visa_Tab = ({ data }) => {
  ...
  const sanitizedHTML = DOMPurify.sanitize(data.singleVisaServiceData?.post_details);
  return (
    <div className=" p-2">
      ...
      {activeTab === "DETAILS" && (
        <div className="">
          <h2 className="text-text_xxl font-bold text-center my-6">
            {data.singleVisaServiceData?.title} <span> </span>Checklist
          </h2>
          <div className="flex flex-col gap-2">
            {data.singleVisaServiceData?.visa_service_document_data?.map(
              (item, index) => (
                <Visit_Visa_Checklist
                  key={index}
                  title={item.title}
                  content={DOMPurify.sanitize(item.post_details)}
      ...
      {activeTab === "BASIC INFORMATION" && (
        <div className="">
          <h2 className="text-text_xxl font-bold text-center mt-6">
            {data.singleVisaServiceData?.title} <span> </span>Basic Information
          </h2>
      ...
      {activeTab === "APPOINTMENT" && (
        <div className="mt-6 text-center">
          ...
          <div>
            <Visit_Visa_Appointment data={data}></Visit_Visa_Appointment>
          </div>
        </div>
      )}
    </div>
