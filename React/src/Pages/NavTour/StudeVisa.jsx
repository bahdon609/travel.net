...
// import CanadaVisaRequirements from "./CanadaVisaRequirements";
...
const StudeVisa = () => {
  ...
  const { data: getstudy_visa = {} } = useQuery({
    queryKey: ["study visa"],
    queryFn: async () => {
      const res = await axiosPublic.get("/get-study-visa-data");
      return res.data;
    },
  });
  //(getstudy_visa);
          ...
          <div>
            <div className="flex flex-col items-center md:mb-6">
              <p className="text-text_30px md:text-text_40px font-bold text-clr_primary_text mb-2">
                <span className="text-clr_primary_text">Study Visa</span>
              </p>
            ...
            <div className="mt-4 lg:mt-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-3 lg:gap-10 mx-auto w-full xl:w-[1300px] mb-5">
              {getstudy_visa?.studyVisaData?.data?.map((visa) => (
                // to={`/azerbaijan-visa-details/${visa.id}`} key={visa.id}
                <Link to={`/study-visa-details/${visa.id}`} key={visa.id}>
