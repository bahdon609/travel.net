...
import NavUmrahSkelator from "./NavUmrahSkelator";
import DOMPurify from "dompurify";
import NavHaj from "../NavHaj/NavHaj";
...
const NavUmrah = () => {
  ...
  const { data = {} } = useQuery({
    queryKey: ["umrah_category"],
    queryFn: async () => {
      const res = await axiosPublic.get("/get-umrah-package-data");
      //("API Response: ", res.data); // Add this to check the API response
      return res.data;
    },
  });
  //(data)
  ...
  return (
    <>
      <div className="z-1">
              ...
              >
                ...
                >
                  <h1 className="text-4xl font-bold mb-4">
                    Umrah Packages 2024-2025 from Bangladesh
                  </h1>
                </div>
              </div>
            </Parallax>
        ...
        <div>
          {loading ? (
            <NavUmrahSkelator /> // Show skeleton loader while loadings
          ) : (
            <div className="mx-auto w-full xl:w-[1300px]">
              <div className="flex flex-col items-center md:mb-6 mt-10">
                <p className="text-text_30px md:text-text_40px font-bold text-clr_primary_text mb-2">
                  <span className="text-clr_primary_text">Umrah Package</span>{" "}
                </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6   py-p_60px">
                {/* Card 1 */}
                {data?.umrahPackageData?.data?.map((packageItem) => (
                  <Link
                    to={`/umrah-details/${packageItem.id}`}
                    key={packageItem.id}
