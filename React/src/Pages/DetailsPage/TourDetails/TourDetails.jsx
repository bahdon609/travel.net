...
const TourDetails = () => {
  ...
  const { data = {}, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic(`/single-tour-package-data/${id}`);
      return res.data;
    },
  });
  //(data);
  ...
  const [mainImage, setMainImage] = useState(
    data?.singleTourPackageData?.photo_1
  );
  const [thumbImages, setThumbImages] = useState([
    data?.singleTourPackageData?.photo_2,
    data?.singleTourPackageData?.photo_3,
  ]);
  ...
  useEffect(() => {
    if (data?.singleTourPackageData) {
      setMainImage(data.singleTourPackageData.photo_1);
      setThumbImages([
        data.singleTourPackageData.photo_2,
        data.singleTourPackageData.photo_3,
      ]);
  ...
  const sanitizedHTML = DOMPurify.sanitize(
    data?.singleTourPackageData?.post_details
  );
  const sanitizedHTML_itinerary = DOMPurify.sanitize(
    data?.singleTourPackageData?.itinerary_details
  );
  const sanitizedHTML_remarks = DOMPurify.sanitize(
    data?.singleTourPackageData?.remarks_details
  );
    ...
    <div className="">
      <div>
        {loading ? (
          ...
          <Parallax
            blur={0}
            bgImage={`${imgUrl}${data?.singleTourPackageData?.photo_1}`}
            ...
            >
              ...
              >
                <h1 className="text-4xl font-bold mb-4">
                  {data?.singleTourPackageData?.title}
                </h1>
              </div>
            </div>
          </Parallax>
          ...
          <div className="flex flex-col md:flex-row mx-auto w-full xl:w-[1300px]  py-p_40px gap-6">
            ...
            <div className="w-full lg:w-[70%]">
              ...
              <div className="w-full mt-m_24px border bg-white shadow-lg rounded-lg">
                ...
                {/* <div>
                            <button
                                className={`w-full p-4 border-b text-left flex justify-between items-center ${activeIndex === 3 ? ' bg-sky_blue_100' : ' bg-white text-black'}`}
                                onClick={() => toggleAccordion(3)}
                            >
                                <span className={`font-bold text-text_title ${activeIndex === 3 ? 'text-black' : 'text-black'}`}>FAQ</span>
                                <span>
                                    {activeIndex === 3 ? (
                            ...
                            {activeIndex === 3 && (
                                <div className="p-4 bg-gray-100">
                                    <p>{stripHtml(data?.singleTourPackageData?.faq_details)}</p>
                                </div>
                            )}
                        </div> */}
              </div>
            ...
            <div className="w-full lg:w-[30%] bg-white">
                ...
                <div className="border p-p_16px ">
                  <p className="text-text_title">
                    ...
                    <span className="font-bold text-sky_blue">
                      {stripHtml(data?.singleTourPackageData?.package_price)}{" "}
                    </span>{" "}
                    Per Person
                  </p>
                  <div className="flex gap-2 flex-wrap">
                    {data?.tourPackageTagsData.map((tag, index) => (
                      <button
                        key={index}
                        className="btn_small_tag  text-text_standard mt-m_medium"
                      >
                        {tag.tag_name}
                      </button>
                    ))}
              ...
              <div className="mt-m_24px">
                <div>
                  ...
                  <div>
                    {data?.moerTourPackageData?.map((pkg, index) => (
                      <Link
